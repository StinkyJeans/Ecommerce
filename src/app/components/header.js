          ...
          <div className="relative flex-1 sm:flex-initial min-w-0" ref={dropdownRef}>
            ...
            >
              ...
              <span className={`font-semibold text-[11px] sm:text-xs md:text-sm lg:text-base truncate min-w-0 flex-1 text-[#2C2C2C] dark:text-[#e5e5e5]`}>
                {loading && !username ? "Loading..." : (username ? `${username.toUpperCase()}'S ACCOUNT` : "Account")}
              </span>
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#2C2C2C] rounded-xl shadow-xl border border-[#E0E0E0] dark:border-[#404040] py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
